...
DECLARE
    ...
    v_from_ledger_account_id INTEGER;
    v_to_ledger_account_id INTEGER;
...
BEGIN
    ...
    UPDATE ledgerr.payment_accounts SET account_id = v_to_ledger_account_id WHERE external_account_id = 'EXT-PAY-002';
    
    -- Setup: Give the source account some initial balance
    v_journal_lines := jsonb_build_array(
        jsonb_build_object('account_id', v_from_ledger_account_id, 'debit_amount', 5000.00, 'description', 'Initial funding'),
        jsonb_build_object('account_id', v_to_ledger_account_id, 'credit_amount', 5000.00, 'description', 'Funding source')
    );
