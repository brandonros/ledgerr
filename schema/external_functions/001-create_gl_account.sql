CREATE OR REPLACE FUNCTION ledgerr_api.create_gl_account(
    p_account_code VARCHAR(10),
    p_account_name VARCHAR(100),
    p_account_type VARCHAR(20),
    p_parent_gl_account_id UUID DEFAULT NULL,
...
BEGIN
    ...
    v_gl_account_id := COALESCE(p_gl_account_id, gen_random_uuid());
    ...
    IF p_parent_gl_account_id IS NOT NULL THEN
        IF NOT EXISTS (SELECT 1 FROM ledgerr.gl_accounts WHERE gl_account_id = p_parent_gl_account_id AND is_active = TRUE) THEN
            RAISE EXCEPTION 'Parent GL account not found or inactive: %', p_parent_gl_account_id;
        END IF;
    END IF;
    
    -- Check for duplicate account code
    IF EXISTS (SELECT 1 FROM ledgerr.gl_accounts WHERE account_code = p_account_code) THEN
